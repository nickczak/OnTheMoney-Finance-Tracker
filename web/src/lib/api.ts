...
//   VITE_API_URL=http://192.168.1.10:8080 npm run dev
...
function normalizeBaseUrl(raw: string): string {
  ...
  try {
    ...
    throw new Error(
      `Invalid VITE_API_URL "${raw}". It must be an absolute URL like http://192.168.1.10:8080.`,
    );
  }
  return url;
}
...
export async function deleteMyAccount(): Promise<void> {
  const res = await authPost("/api/auth/delete-account", {
    token: requireToken(),
  });
  if (!res.ok) throw new Error(await errorMessage(res));
  await clearSession();
...
export async function addToWatchlist(symbol: string): Promise<void> {
  const res = await apiFetch(
    `/api/stocks/watchlist?symbol=${encodeURIComponent(symbol)}`,
    {
      method: "POST",
    },
  );
...
export async function removeFromWatchlist(symbol: string): Promise<void> {
  const res = await apiFetch(
    `/api/stocks/watchlist/${encodeURIComponent(symbol)}`,
    {
      method: "DELETE",
    },
  );
...
export async function projectRetirement(
  ...
  const res = await apiFetch(`/api/project?${params.toString()}`, {
    method: "POST",
  });
  if (!res.ok) throw new Error(`HTTP ${res.status}`);
  return res.json();
